...
app.config['MYSQL_PASSWORD'] = 'O6MZakN64tMJafE'
app.config['MYSQL_DB'] = 'residencial'
...
@app.route('/crearRegistroManual', methods=['POST'])
@jwt_required()
def crearRegistroManual():
    data = request.json
    ...
    cur = mysql.connection.cursor()
    try:
        cur.execute('INSERT INTO `historial_manual`(`numberId`, `name_lastname`, `destination`, `patent`, `comments`) VALUES (%s, %s, %s, %s, %s)', (numberId, name_lastname,destination, patent, comments))
        mysql.connection.commit()
        return jsonify({'message': 'Se ingresó correctamente'}), 201
    except Exception as e:
        mysql.connection.rollback()
